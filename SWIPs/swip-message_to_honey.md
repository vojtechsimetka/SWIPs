...
* The node will actively keep track of the retirement of prices and will query the price oracle on a predifined x-duration (e.g. 10 hours) before the expected release of a new price to pre-load the new-price in his local database. New price will be published into the oracle prior to the expiry of the last price such that each node has to check the price oracle at least twice (=the announcencement of new price is at least 2x of price' time to live period).
